...
cross-track event rows in patient-report sections 5 and 6, with at most three
ranked peptide-HLA candidates per event; `--top-n` continues to control the
first-batch experiment set only.
...
4. Review event-level representatives, with at most three peptide-HLA pairs per event and explicit phase/redundancy handling. Patient-report sections 5 and 6 must be organized by variant/event rather than by flat peptide rows; render candidate peptide-HLA pairs in three bounded columns so overlapping windows or multiple HLA predictions cannot over-represent one event.
